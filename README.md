...
Following flags should be used in `CURR_OPT_CONF`:
...
`/mnt/vmoutput/tpcds-15tb-full-gen1.profile` should be the _second generation ReadyNow profile_, collected as output profile (`-XX:ProfileLogOut`) from run which was had first generation profile as input, etc. See [Training ReadyNow](https://docs.azul.com/prime/Use-ReadyNow-Training) for more details, and look at the flags for the intermediate runs in `config/15tb-full-run-zing.profile`.

Output profiles are generated by all executors, one ReadyNow profile file should be chosen and used as input for the next run. Input profile can be downloaded e.g. as part of bootstrap, see `bootstrap.sh.tpl`.
